...
def _esc(value) -> str:
    """Escape ký tự đặc biệt HTML — dữ liệu người dùng (tên cột, tên nhóm...)
    có thể chứa <, >, & ... nếu không escape sẽ làm vỡ layout báo cáo."""
...
def _group_table_html(by_group: dict) -> str:
    ...
    for key, s in by_group.items():
        rows.append(
            f"<tr><td>{_esc(key)}</td><td>{int(s.get('count_valid', 0)):,}</td>"
...
def build_html_report(
    ...
    file_name_safe = _esc(file_name)
    column_safe = _esc(column)
    group_by_safe = _esc(group_by) if group_by else ""
    ...
    for title, data_uri in charts.items():
        ...
        <div class="chart-block">
            <img src="{data_uri}" alt="{_esc(title)}" />
        </div>
        """
    ...
    if group_by and "by_group" in result:
        group_section = f"""
        <h2>Thống kê theo &quot;{group_by_safe}&quot;</h2>
...
<head>
<meta charset="UTF-8" />
<title>Báo cáo thống kê dữ liệu - {file_name_safe}</title>
<style>
    :root {{
        --accent: #3457D5; --accent-strong: #22348C; --accent-soft: #E4EAFC;
        --ink: #232B3D; --ink-soft: #6B7690; --border: #E1E5F0; --paper-2: #F5F7FC;
    }}
    body {{
    ...
    section {{
        background: white; border: 1px solid var(--border); border-radius: 10px; padding: 22px 26px;
        margin-bottom: 22px;
    }}
    h2 {{ color: var(--accent-strong); border-bottom: 2px solid var(--border); padding-bottom: 8px; }}
    table.stats-table {{
    ...
    table.stats-table th {{ background: var(--paper-2); }}
    table.stats-table tr:nth-child(even) {{ background: #FAFBFE; }}
    .chart-block {{ margin: 18px 0; text-align: center; }}
    .chart-block img {{ max-width: 100%; border-radius: 6px; border: 1px solid var(--border); }}
...
<header>
    ...
    <p>Tệp dữ liệu: <strong>{file_name_safe}</strong> &nbsp;|&nbsp; Cột phân tích: <strong>{column_safe}</strong></p>
